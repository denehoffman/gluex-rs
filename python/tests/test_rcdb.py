...
def test_fetch_with_filters(db: rcdb.RCDB) -> None:
    data = db.fetch(
        ...
        run_max=1100,
        filters=rcdb.all(
            rcdb.string_cond('run_type').isin(['hd_all.tsg', 'hd_all.tsg-m8', 'hd_all.tsg-m7']),
...
def test_fetch_runs_with_alias(db: rcdb.RCDB) -> None:
    runs = db.fetch_runs(run_min=10000, run_max=10300, filters=rcdb.aliases.is_production)
    assert runs
