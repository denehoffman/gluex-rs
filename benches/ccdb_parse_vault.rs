...
use gluex_rs::ccdb::{data::ColumnLayout, database::CCDB, models::ColumnMeta};
...
fn bench_parse_vault(c: &mut Criterion) {
    ...
    c.bench_function("parse_vault_test_table", |b| {
        b.iter(|| {
            let data =
                gluex_rs::ccdb::data::Data::from_vault(black_box(&first), layout.clone(), n_rows)
                    .expect("parse failed");
            black_box(data);
        });
    });
...
fn bench_parse_multiple_vaults(c: &mut Criterion) {
    ...
    c.bench_function("parse_multiple_vaults_test_table", |b| {
        b.iter(|| {
            for vault in &vaults {
                let data = gluex_rs::ccdb::data::Data::from_vault(
                    black_box(vault),
                    layout.clone(),
                    n_rows,
                )
