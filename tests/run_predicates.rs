...
fn composed_predicates_follow_the_three_valued_truth_table() {
    ...
    for (a, b, and, or) in cases {
        for (predicate, expected) in [
            ...
            let result = base.filter(predicate).collect().unwrap();
            assert_eq!(
                result.numbers(),
...
fn approval_is_explicit_and_definition_operands_are_parameterized() {
    ...
    assert_eq!(
        base.filter(gluex_rs::approved_production(gluex_rs::RunPeriod::RP2018_01).unwrap())
            .collect()
            .unwrap()
            .numbers(),
        &[50685, 50697]
    ...
    let d = gx.conditions().unwrap();
    assert!(
        base.filter(d["daq_run"].eq("PHYSICS' OR 1=1 --").unwrap())
            .collect()
            .unwrap()
            .numbers()
            .is_empty()
    ...
    assert_eq!(
        gx.runs(RunSelection::runs([2, 3, 4]))
            .unwrap()
            .filter(d["event_count"].ge(1686).unwrap() & d["event_count"].le(5000).unwrap())
            .collect()
            .unwrap()
            .numbers(),
        &[3, 4]
    ...
    assert_eq!(
        gx.runs(RunSelection::runs([2, 3]))
            .unwrap()
            .filter(d["run_start_time"].eq(time).unwrap())
            .collect()
            .unwrap()
            .numbers(),
        &[2]
