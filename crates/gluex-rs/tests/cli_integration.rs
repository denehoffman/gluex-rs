...
fn assert_histograms_close(actual: &Histogram, expected: &Histogram) {
    assert_relative_eq!(
        actual.counts.as_slice(),
        expected.counts.as_slice(),
        max_relative = 8.0 * f64::EPSILON
    );
    assert_relative_eq!(actual.edges.as_slice(), expected.edges.as_slice());
    assert_relative_eq!(
        actual.errors.as_slice(),
        expected.errors.as_slice(),
        max_relative = 8.0 * f64::EPSILON
    );
}
