...
fn open_db() -> CCDB {
    CCDB::open(db_path()).expect("failed to open CCDB test database")
}

#[test]
fn parse_timestamp_infers_and_validates_inputs() {
    let ts = parse_timestamp("2013-02-22 19:40:35").unwrap();
...
#[test]
fn directory_and_table_metadata_can_be_discovered() {
    let db = open_db();

    let root = db.root();
    assert_eq!(root.full_path(), "/");

    let test_dir = db.dir("/test").unwrap();
    assert_eq!(test_dir.full_path(), "/test");

    let demo_dir = test_dir.dir("demo").unwrap();
    assert_eq!(demo_dir.full_path(), "/test/demo");

    let table = demo_dir.table("mytable").unwrap();
...
#[test]
fn fetch_respects_runs_variations_and_timestamps() {
    let db = open_db();
    let before_first = parse_timestamp("2013-02-22 19:40:34").unwrap();
    let first_available = parse_timestamp("2013-02-22 19:40:35").unwrap();
    let updated = parse_timestamp("2020-02-01 00:00:00").unwrap();

    let empty_ctx = Context::default()
        .with_run_range(0..=3)
        .with_timestamp(before_first);
    let empty = db.fetch(TABLE_PATH, &empty_ctx).unwrap();
    ...
    let mc_ctx = Context::default()
        ...
        .with_variation("mc")
        .with_timestamp(first_available);
    let mc_result = db.fetch(TABLE_PATH, &mc_ctx).unwrap();
