...
use laddu::physics::quantum::{ExternalId, ParticleProperties};
...
impl FromStr for Particle {
    type Err = crate::core::GlueXCoreError;
    ...
    fn from_str(name: &str) -> Result<Self, Self::Err> {
        Self::VARIANTS
            ...
            .ok_or_else(|| crate::core::GlueXCoreError::ParticleParse(name.to_owned()))
    }
}
...
impl From<ParticleProperties> for Particle {
    fn from(properties: ParticleProperties) -> Self {
        let mut res = Self::UnknownParticle;
        for (namespace, id) in properties.ids().iter() {
            match (namespace.to_lowercase().as_str(), id) {
                ("pdg", ExternalId::Code { value }) => res = Self::from_pdg(*value as isize),
                ("geant" | "geant4", ExternalId::Label { value }) => {
                    if let Some(particle) = Self::from_geant4(value) {
                        res = particle;
                    }
                }
                _ => {}
