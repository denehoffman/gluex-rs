...
use crate::core::GlueXCoreError;

/// Parses a timestamp string into a [`DateTime`] in the [`Utc`] timezone, inferring missing fields.
///
/// # Errors
///
/// Returns [`GlueXCoreError::TimestampNoDigits`] or [`GlueXCoreError::TimestampChrono`] if parsing fails.
pub fn parse_timestamp(input: &str) -> Result<DateTime<Utc>, GlueXCoreError> {
    let digits: Vec<i32> = input
        ...
        .collect();
    if digits.is_empty() {
        return Err(GlueXCoreError::TimestampNoDigits(input.to_string()));
    }
    let year = digits[0];
    ...
    let date = NaiveDate::from_ymd_opt(year, month, day).ok_or_else(|| {
        GlueXCoreError::TimestampChrono(format!("invalid date: {year}-{month}-{day}"))
    })?;
    let time = NaiveTime::from_hms_opt(hour, minute, second).ok_or_else(|| {
        GlueXCoreError::TimestampChrono(format!("invalid time: {hour}:{minute}:{second}"))
    })?;
