...
impl PyHistogram {
    ...
    fn new(counts: Vec<f64>, edges: Vec<f64>, errors: Option<Vec<f64>>) -> PyResult<Self> {
        let mut histogram =
            Histogram::new(counts, edges).map_err(|err| PyValueError::new_err(err.to_string()))?;
    ...
    fn counts(&self) -> Vec<f64> {
        self.0.counts().to_vec()
    }

    #[getter]
    fn edges(&self) -> Vec<f64> {
        self.0.bin_edges().to_vec()
    }
...
pub(crate) fn histogram_to_dict(py: Python<'_>, histogram: &Histogram) -> PyResult<Py<PyDict>> {
    ...
    dict.set_item("counts", histogram.counts())?;
    dict.set_item("edges", histogram.bin_edges())?;
    dict.set_item("errors", histogram.errors())?;
    Ok(dict.unbind())
}
